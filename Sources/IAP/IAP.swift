...
final public class IAP {
    ...
    public func fetchAvailableProductsInfo() {
        ...
        SwiftyStoreKit.retrieveProductsInfo(products) { [weak self] result in
            ...
            for product in result.retrievedProducts {
                ...
                let requiredProduct = IAPProduct(localizedPrice: price, term: self.getPaymentTerm(identifier: identifier), identifier: identifier, amount: product.price.doubleValue, currencyCode: currencyCode)
    ...
    public func purchase(product: IAPProduct, quantity: Int = 1) {
        productPurchaseState.send(.started)
        SwiftyStoreKit.purchaseProduct(product.identifier, quantity: quantity, atomically: true) { [weak self] (result) in
            guard let self = self else { return }
